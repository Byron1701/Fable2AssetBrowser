...
struct PropInstance {
    ...
    uint32_t record_file_offset = 0;
    uint32_t count_file_offset = 0;
...
struct PendingAdjacentTerrain {
    ...
    std::vector<VistaPatchGeom> patch_geoms;
};
}
...
extern std::unordered_map<uint32_t, Gdb::EntityContents>
    g_level_entity_contents;
extern std::unordered_map<uint32_t, Gdb::EntityGameplayDetails>
    g_level_entity_gameplay;
extern std::unordered_map<uint32_t, Gdb::PropertyDetails>
...
extern std::vector<Gdb::CreatureCatalogEntry> g_global_entity_catalog;
extern std::unordered_map<uint32_t, std::vector<uint32_t>>
    g_global_entity_model_hashes;
extern uint64_t g_global_entity_catalog_revision;
